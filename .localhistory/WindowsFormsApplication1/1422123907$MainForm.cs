    ...
    public partial class Form1 : Form
    {
        ...
        public bool loadEclipseFilesFromLocalDisk()
        {
            ...
            foreach (EclipseObject obj in FILE_MAP)
            {
                ...
                {
                    INI_LIST.Add(obj);
                    ECL_OBJ_MAP.Add(obj.FILE_PATH, obj);
                }
                if (obj.FILE_TYPE == ".ECL")
                {
                    ECL_LIST.Add(obj);
        ...
        public void backupEssentialUserFiles(EclipseObject obj, string destination)
        {
            writeINIbackup(destination + "\\" + obj.FILE_NAME, obj.INI_INFO_ARRAY);            
            string[] filePathArray = new string[] { obj.INI_MAIN_DICTIONARY, obj.INI_SPELL_DIX };
            foreach (string i in filePathArray)
            {
                copyFile(i, obj.INI_JOB_PATH, destination);
            }
            DirectoryCopy(obj.INI_BLOCK_PATH, destination + "\\" + obj.INI_BLOCK_FOLDER + "\\", true);
        }
        ...
        public void copyFile(string sFile, string sLocation, string sDestLocation)
        {
            ...
            try
            {
                ...
                MessageBox.Show("File " + sFile + " backed up to " + targetPath,
               "Backup File Complete",
               MessageBoxButtons.OK);
            }
            catch (System.UnauthorizedAccessException)
            {
                MessageBox.Show("Problem copying: Access Denied when copying:  " + sFile + " to: " + targetPath,
                                "Problem Copying",
                                MessageBoxButtons.OK);
            }
            catch (System.IO.IOException)
            {
                MessageBox.Show("Looks like " + sFile + " is in use by another program.");
            }
        }
        ...
        {
            if (loadEclipseFilesFromLocalDisk()) unpack();
        }
