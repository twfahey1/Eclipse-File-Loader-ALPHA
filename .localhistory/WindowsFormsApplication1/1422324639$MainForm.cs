...
namespace WindowsFormsApplication1
{
    ...
    ///
    {
        ...
        public void backupAllUserFiles(EclipseObject userIniObject, string destination)
        {
            ...
            {
                Directory.CreateDirectory(Path.Combine(destination, userIniObject.INI_JOB_FOLDER, dirPath));
                foreach (string file in Directory.GetFiles(dirPath))
                {
                    copyFile(Path.GetFileName(file), Path.GetDirectoryName(file), dirPath);
                }
            }
