...
namespace WindowsFormsApplication1
{
    ...
    ///
    {
        ...
        public bool RestoreEclipseFilesToLocalPC(EclipseObject iniObject)
        {
            ...
            {
                Directory.CreateDirectory(Path.Combine(destination, iniObject.INI_JOB_FOLDER, subdirPath));
                foreach (string file in Directory.GetFiles(subdirPath))
                {
                    Console.WriteLine("3rd wave - Attempting to copy: " + Path.GetFileName(file) + " / " + Path.GetDirectoryName(file) + " / " + Path.Combine(destination, iniObject.INI_JOB_FOLDER));
                    copyFile(Path.GetFileName(file), Path.GetDirectoryName(file), Path.Combine(destination, iniObject.INI_JOB_FOLDER, Path.GetDirectoryName(subdirPath)));
                    transferProgressBar.PerformStep();
                }
