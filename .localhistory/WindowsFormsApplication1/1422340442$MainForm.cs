...
namespace WindowsFormsApplication1
{
    ...
    ///
    {
        ...
        public bool RestoreEclipseFilesToLocalPC(EclipseObject iniObject)
        {
            ...
            {
                string blockDestination = Path.Combine(destination, iniObject.INI_JOB_FOLDER, new FileInfo(iniObject.INI_BLOCK_PATH).Directory.Name);
                Directory.CreateDirectory(blockDestination);
                foreach (string file in Directory.GetFiles(subdirPath))
                {
                    Console.WriteLine("3rd wave - Attempting to copy: " + Path.GetFileName(file) + " / " + Path.GetDirectoryName(file) + " / " + blockDestination);
                    copyFile(Path.GetFileName(file), Path.GetDirectoryName(file), blockDestination);
                    transferProgressBar.PerformStep();
                }
