    ...
    ///
    {
        ...
        public class EclipseObject
        {
            ...
            public EclipseObject(string name, string type, string path)
            {
                ...
                this.FILE_USER_FOLDER = FILE_PATH.TrimEnd(FILE_NAME.ToCharArray());//trims off the actual file name to reveal the folder
        ...
        //info we get from eclipse.ini, MainDirectory5= line
        public bool RestoreAllEclipseFilesToLocalPC(EclipseObject iniObject)
        {
            transferProgressBar.Value = 0;
            transferProgressBar.Maximum = Directory.GetFiles(Path.GetDirectoryName(iniObject.FILE_PATH) + iniObject.INI_JOB_FOLDER, "*.*", SearchOption.AllDirectories).Length + 1;
            //First use our writeINIBackup to get an ini and a set file created right on the main dir 5 from the eclipse.ini
            ...
            string destination = CURRENT_MAINDIRECTORY5 + "\\" + iniObject.INI_JOB_FOLDER;
            //Now, we launch the copy procedure.
            string[] eclFiles = Directory.GetFiles(Path.GetDirectoryName(iniObject.FILE_PATH) + iniObject.INI_JOB_FOLDER, "*.ecl", SearchOption.TopDirectoryOnly);
            string[] dixFiles = Directory.GetFiles(Path.GetDirectoryName(iniObject.FILE_PATH) + iniObject.INI_JOB_FOLDER, "*.dix", SearchOption.TopDirectoryOnly);
            string[] spellFiles = Directory.GetFiles(Path.GetDirectoryName(iniObject.FILE_PATH) + iniObject.INI_JOB_FOLDER, "*.es*", SearchOption.TopDirectoryOnly);
            string[] wavFiles = Directory.GetFiles(Path.GetDirectoryName(iniObject.FILE_PATH) + iniObject.INI_JOB_FOLDER, "*.wav", SearchOption.TopDirectoryOnly);
            string[] allFiles = Directory.GetFiles(Path.GetDirectoryName(iniObject.FILE_PATH) + iniObject.INI_JOB_FOLDER, "*.*", SearchOption.AllDirectories);
            transferProgressBar.Maximum = Directory.GetFiles(Path.GetDirectoryName(iniObject.FILE_PATH) + iniObject.INI_JOB_FOLDER, "*.*", SearchOption.AllDirectories).Length;
        ...
        {
            transferProgressBar.Value = Directory.GetFiles(userIniObject.INI_JOB_PATH, "*", SearchOption.AllDirectories).Length+1;
            writeINIbackup(destination + "\\" + userIniObject.FILE_NAME, userIniObject.INI_INFO_ARRAY);
            transferProgressBar.PerformStep();
            ...
            foreach (string i in Directory.GetFiles(userIniObject.INI_JOB_PATH, "*", SearchOption.AllDirectories))
                try
                {
        ...
        {
            if (RestoreAllEclipseFilesToLocalPC(ECL_OBJ_MAP[currentUsersDropdown.Text])){
