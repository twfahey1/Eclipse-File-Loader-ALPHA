    ...
    ///
    {
        ...
        public void backupEssentialUserFiles(EclipseObject userIniObject, string destination)
        {
            ...
            foreach (string i in checkedFilesToCopy)
                ...
                try
                {
                    foreach (string jobfile in Directory.EnumerateFiles(userIniObject.INI_JOB_PATH, "*", SearchOption.AllDirectories).Where(s => s.StartsWith(i)))
                    {
                        Console.WriteLine("jobile in Copystring: " + jobfile + " \\ " + jobfile);
